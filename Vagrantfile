...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-END
	...
	apt-get update #packet manager index bijwerken
	
	#begin mysql installatie
	debconf-set-selections <<< "mysql-server mysql-server/root_password password $MYSQL_PASSWORD"		#mysql installatie instellingen
	...
	debconf-set-selections <<< "phpmyadmin phpmyadmin/mysql/app-pass password $MYSQL_PASSWORD"			#phpmyadmin installatie instellingen
	...
	npm install -g --unsafe-perm loopback-cli
	...
	#begin angular installatie
	npm install -g --unsafe-perm @angular/cli
	#einde angular installatie
  END
end
